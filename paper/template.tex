...
\end{minipage}
\begin{minipage}{.4\textwidth}
In the first for loop we iterate the network layer wise and collect nodes falling under prune criteria and store it in \var{prune nodes}. \newline We then reverse iterate the network to change layer weights, biases and connection weights based on \var{prune nodes}\newline\newline
The pruner function takes a layer and the corresponding prune indices and calculates new layer dimensions. It also clones the layer weights, layer bias, connection weights and extracts appropriate indices to match the layer dimensions of the new layer. Finally these new set of parameters are assigned to layer created in the previous step.
\end{minipage}
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.5]{paper/100p_runs.png}
