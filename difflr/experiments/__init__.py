...
import json
from difflr.utils import CustomJsonEncoder, generate_timestamp
                ...
                for batch_size in self.config['batch_size']:
                    ...
                    for lr in self.config['lr']:
                        ...
                        if 'best_metrics' in self.best_params:
                            ...
                            self.best_params['best_metrics'] = metrics
                        print("====" * 25, "\n\n")
        json.dump(self.best_params, open(DIFFLR_EXPERIMENTS_RUNS_PATH + f'/tuner_{generate_timestamp()}.json', 'w'), cls=CustomJsonEncoder, indent=2)
